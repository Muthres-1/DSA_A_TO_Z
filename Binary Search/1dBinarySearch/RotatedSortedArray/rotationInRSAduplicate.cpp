...
while (left<=right)
{
    ...
    // Just adding this if condition we can solve for duplicates also 
    if(arr[left] == arr[mid] && arr[mid] == arr[right]){
        left++;
        right--;
        index--; // because left is shifting so it will show effect but right doesn't show effect
        continue;
    }
...
int main()
{
vector<int>arr={3,3,4,4,5,1,2}; // This is rotated 3 times 
